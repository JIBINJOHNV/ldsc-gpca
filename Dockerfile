...
ENV PATH="/opt/environments/main/bin:/opt/environments/conda/bin:${PATH}" \
    ...
    TZ="Etc/UTC"
RUN useradd --create-home --uid 10001 analysis && mkdir /work && chown analysis /work
USER analysis
WORKDIR /work
ENTRYPOINT ["/bin/bash", "-c", "source /opt/environments/activate.sh && exec ldsc-gpca \"$@\"", "ldsc-gpca"]
CMD ["--help"]
